 ...
 * O terceiro parametro serve para informar qual indice se trata */
	 ...
	 if(ip == 'p'){
		 ...
		 int tam = 0;
			 for(int i = 0; i < aux->num_chaves; i++){
				 sprintf((tempHash + tam), "%s", aux->chave[i].pk);
				 tam += 10;
				 sprintf((tempHash + tam),"%04d", aux->chave[i].rrn);
				 tam += 4;
 ...
 void *read_btree(int rrn, char ip){
	 ...
	 if(ip == 'p'){
		 ...
		 //TODO loop aqui ou na func cadastro?
...
void libera_no(void *node, char ip){
 ...
 void inserir_registro_indices(Indice *iprimary, Indice *ibrand, Produto product){
	 if(iprimary->raiz == -1){//caso o indice esteja vazio, criamos o primeiro indice
		...
		novoIp->chave[0].rrn = nregistrosip; //rrn do produto no arquivo de dados
		iprimary->raiz = 0;
	...
	if(ibrand->raiz == -1){
		...
		if(promocaoIs){
			...
			sprintf(y->chave[0].string, "%s$%s", product.marca, product.nome);
	 ...
	 if(x->folha == 'F'){
		 ...
		 if(x->num_chaves < (ordem_ip - 1)){
			 ...
			 return NULL;
			 	...
			 	libera_no(x, 'p');
	   			libera_no(node, 'p');
				return divide_no_ip(iprimary, product, -1);
		 	}
		}
...
//insercao na arvore b, indice is (pseudo codigo de insercao em arvore b)
 ...
 Info_node* divide_no_ip(Indice* x, Produto product, int fdireito){
	...
	write_btree(y, nregistrosip, 'p');
	 nregistrosip++;//TODO isso esta certo antes do segundo write?
	 libera_no(node, 'p');
	 libera_no(y, 'p');
